...
timeStack.onSelect = function() {
    ...
    for (var a in fileList) {
        ...
        if (Number(cameraSerial > 100 && Number(dateTime) > 1)) fileArray.push([
            [decodeURI(fileList[a].name)],
            [dateTime],
            [cameraSerial]
        ]);
    }
    ...
    for (var s in fileArray) {
        serNos.push(Number(fileArray[s][2]));
    }
    serNos = uniqueSortedList(serNos);
    var Camera = new Object();
    for (var g in serNos) {
        Camera[serNos[g]] = new Array();
    };
    for (var d in fileArray) {
        Camera[Number(fileArray[d][2])].push([
            [fileArray[d][0].toString()],
            [Number(fileArray[d][1])]
        ]);
    }
    fileArray = [];
    for (var k in serNos) {
