...
expertSearch.onSelect = function() {
    ...
    for (var a in ns) {
        ...
        if (parts.length == 2) NameSpaces.push([
            [parts[0].toString().replace(/^\s+|\s+$/, "")],
            [parts[1].toString().replace(/^\s+|\s+$/, "")]
        ]);
    }
    ...
    win.g100.bu1.onClick = function() {
        ...
        if (win.g20.cb1.value) var patt = new RegExp(win.g10.et1.text.toString(), "i")
        else var patt = new RegExp(win.g10.et1.text.toString());
            ...
            while ((prop = iter.next()) != null) {
                if (prop.path != null) {
                    if (patt2.test(prop.path.toString())) {
                        if (patt.test(prop.value.toString())) {
                            thumbList.push(items[p]);
                        }
                    }
                }
            }
